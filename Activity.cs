   ...
   public class Activity
    {
        ...
        public Model.Activity GetModel(int id)
        {
            ...
            {
                string sql = "select activityId,activityName,endTime,activityPicture," +
                    "summary,activityVerify,userName,recommend,recommendtime," +
                    "activityIntroduction,endTime>=now() as activityStatus " +
        ...
        public int GetVerifyCount()
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "select count(1) form activity where activityVerify='审核通过'";
        ...
        public IEnumerable<Model.Activity> GetVerifyPage(Model.Page page)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by endTime desc)" +
                    " as num, activityId,activityName,endTime,activityPicture,summary," +
                    "activityVerify,userName,recommend,recommendtime," +
                    "endTime>=now() as activityStatus from activity where activityVerify='审核通过')";
        ...
        public Model.Activity GetRecommend()
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVeryfy='审核通过'and recommend='是' and endTime>=now() order by recommendTime desc limit 1";
        ...
        public IEnumerable<Model.ActivityNo> GetPage(Model.Page page)
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number over endTime desc) as num,activity * from activity)";
        ...
        public int Add(Model.Activity active)
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "insert into activity(activityname,endtime,activitypicture,"
                    + "activityintroduction,summary," + "activityverify,activitystatus," +
                    "username,recommend) values(@activityName, @endTime, @activityPicture, " +
                    "@activityIntroduction, @summary, @activityVerify, @activityStatus, @userName, @recommend);";
                sql += "SELECT @@IDENTITY";
        ...
        public int UpdataImg(Model.Activity active)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
        ...
        public int UpdateVerify(Model.Activity active)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "update activity set activityVerfy=@activityVerfy where activityid=@activityId";
                return cn.Execute(sql, active);
            }
        }
